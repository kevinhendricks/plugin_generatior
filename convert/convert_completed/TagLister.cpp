...
void TagLister::parseAttribute(const QStringView tagstring, const QString &attribute_name, AttInfo &ainfo)
{
    ...
    while (tagstring.indexOf(QChar('='), p) != -1) {
        ...
        QString aname = Substring(s, p, tagstring).trimmed();
        ...
        if ((tagstring.at(p) == '\'') || (tagstring.at(p) == '"')) {
            ...
            avalue = Substring(b, p, tagstring);
            if (aname == attribute_name) {
                ainfo.avalue = avalue;
                ainfo.len = p - s + 1;
                ainfo.vpos = b;
...
QStringView TagLister::parseML()
{
    ...
    if (m_source.at(p) != '<') {
        ...
        return SubstringView(m_pos, m_next, m_source);
    }
...
void TagLister::parseTag(const QStringView tagstring, TagLister::TagInfo& mi)
{
    ...
    int b = p;
    p = stopWhenContains(tagstring, ">/ \f\t\r\n", p);
    mi.tname = Substring(b, p, tagstring);
...
QString TagLister::Substring(int start_index, int end_index, const QStringView string)
{
...
// the characters included are in the interval:
// [ start_index, end_index >
QString TagLister::Substring(int start_index, int end_index, const QString &string)
{
...
//static
// Returns a substring of a specified QString as a QStringView
// the characters included are in the interval:
// [ start_index, end_index >
QStringView TagList::SubstringView(int start_index, int end_index, const QString &string)
{
